...
#Receiver is a thread that starts a server socket, receives messages and resolves them (decides whether it’s a new user or an actual message)
class Receiver(threading.Thread):
    ...
    def resolve_message(self, message):
        if "connect_request" in message:
            ...
            arr = message.split("|") #we defined a standard where a new user should send a message containing “connect_request|-user_name-|-client_host-|-client_port-“, and a message should contain “from:-user_name-|-message_text-“. Every client that meets this standards will be able to use our chat room without any problem.
            if len(arr) == 4:
                user_name = arr[1]
                user_host = arr[2]
                user_port = arr[3]
    ...
    def run(self):
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #start server socket
            ...
            try:
                ...
                while True:
                    ...
                    if not data:
                        ...
                        self.resolve_message(full_message.strip()) #will decide whether it’s a new user or an actual message that it has to put in our queue to forward it to all connected users.
                        break
            finally:
...
class Sender(threading.Thread):
    ...
    def send(self, message, host, port):
        print("sending message {} to {}".format(mesage, str((host, port))))
    ...
    def run(self):
        while True:
            if self.queue.messages_waiting():
                print("there are messages on queue. popping")
